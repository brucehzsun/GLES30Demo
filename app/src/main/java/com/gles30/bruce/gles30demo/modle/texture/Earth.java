...
public class Earth {
    ...
    public void initShader(Context context) { //初始化着色器
        //加载顶点着色器的脚本内容
        String mVertexShader = ShaderUtil.loadFromAssetsFile(context, "texture/vertex_earth.sh");
        //加载片元着色器的脚本内容
        String mFragmentShader = ShaderUtil.loadFromAssetsFile(context, "texture/frag_earth.sh");
        //基于顶点着色器与片元着色器创建程序
    ...
    public void drawSelf(int texId, int texIdNight) {
        ...
        //将光源位置传入渲染管线
        GLES30.glUniform3fv(maSunLightLocationHandle, 1, MatrixState.getLightPositionFloatBuffer());
